...
const Add = ({ isOpen, onClose, onChannelAdded }) => {
  ...
  const formatConditions = useCallback(() => {
    ...
    return [];
  }, [conditions, contactType.type]);

  const create = async () => {
    const response = await post(API.channels.intercept(), null, {
      ...values,
      config: {
            ...
            <Card className={style.detailsCard}>
              <header>
                <span className={style.title}>
                  Deactivate Instance of meeting type
                </span>
              </header>
              <div className="row">
                <div className="col-12">
                  <div className="form--inline">
                    ...
                    <span className={style.inlineLbl}>When</span>
                    <label htmlFor="name">
                      {salesforce ? (
                        <Dropdown
                          label="Select..."
